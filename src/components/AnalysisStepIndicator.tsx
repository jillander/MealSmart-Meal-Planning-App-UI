...
import { CloudUploadIcon, SearchIcon, DatabaseIcon, CheckCircleIcon, RefreshCwIcon } from 'lucide-react';
interface Step {
  id: number;
  name: string;
  message: string;
...
export const AnalysisStepIndicator: React.FC<AnalysisStepIndicatorProps> = ({
  ...
  const getStepIcon = (stepName: string, isActive: boolean, isComplete: boolean, hasError: boolean) => {
    const iconSize = 18;
      ...
      <div className="flex items-center justify-between">
        ...
        return <div key={step.id} className="flex flex-col items-center relative">
              ...
              <div className={`w-10 h-10 rounded-full flex items-center justify-center
                ...
                {isError ? <button onClick={onRetry} className="text-red-500 hover:text-red-600">
                    <RefreshCwIcon size={18} />
                  </button> : getStepIcon(step.name, isActive, isComplete, hasError)}
              </div>
              {/* Step Name */}
              ...
              {/* Connector Line */}
              {index < steps.length - 1 && <div className={`absolute top-5 left-10 w-[calc(100%-1rem)] h-0.5
                    ${index < currentStep ? 'bg-[#4CAF50]' : 'bg-gray-200'}
                    transition-colors duration-250`} style={{
            width: 'calc(100% + 1rem)'
          }} />}
            </div>;
      })}
