...
import { XIcon, ChevronDownIcon, ChevronUpIcon, RefreshCwIcon, PlayIcon, SkipForwardIcon } from 'lucide-react';
...
export const IngredientAnalysisModal: React.FC<IngredientAnalysisModalProps> = ({
  isOpen,
  onClose,
  onComplete,
  testMode = false
  ...
  const startAnalysis = () => {
    ...
    steps.forEach((step, index) => {
      ...
      setTimeout(() => {
        ...
        const progressPerInterval = step.duration / intervalTime / (totalDuration / 100);
        const progressInterval = setInterval(() => {
          setProgress(prev => {
            const newProgress = Math.min(prev + progressPerInterval, accumulatedProgress + step.duration / totalDuration * 100);
            return newProgress;
          });
        }, intervalTime);
  ...
  return <div className="fixed inset-0 z-50 flex items-end justify-center">
      ...
      <div className="relative w-full max-w-[430px] bg-white rounded-t-[24px] shadow-xl transform transition-transform duration-300 ease-out animate-slide-up">
        ...
        {/* Test Mode Indicator */}
        {testMode && <div className="absolute top-2 right-2">
        ...
        <div className="pt-6 px-6 flex justify-between items-center">
          ...
          <button onClick={onClose} className="w-8 h-8 flex items-center justify-center rounded-full text-[#757575] hover:bg-gray-100">
            <XIcon size={20} />
          </button>
        </div>
        {/* Progress Circle */}
        <div className="flex justify-center mt-6 mb-4">
          <AnalysisProgressCircle progress={progress} size={64} hasError={hasError} />
        </div>
        {/* Steps Indicator */}
        <div className="px-6 mt-2">
          <AnalysisStepIndicator steps={steps} currentStep={currentStep} hasError={hasError} onRetry={retryAnalysis} />
        </div>
        {/* Test Mode Controls */}
        {testMode && <div className="px-6 mt-6">
            <div className="p-3 bg-red-50 rounded-lg border border-red-200">
              <h3 className="text-sm font-medium text-red-800 mb-2">
                Test Mode Controls
              </h3>
              <div className="flex space-x-2">
                <button onClick={goToNextStep} className="flex-1 py-2 px-3 bg-red-600 text-white text-sm rounded-lg flex items-center justify-center">
                  <PlayIcon size={14} className="mr-1" />
                  Next Step
                </button>
                <button onClick={skipToCompletion} className="flex-1 py-2 px-3 bg-red-600 text-white text-sm rounded-lg flex items-center justify-center">
        ...
        <div className="px-6 mt-6 pb-6 border-t border-gray-100 pt-4">
          <button onClick={() => setShowDetails(!showDetails)} className="flex items-center text-sm text-[#757575] font-medium">
            {showDetails ? <ChevronUpIcon size={16} className="mr-1" /> : <ChevronDownIcon size={16} className="mr-1" />}
          ...
          {/* Error State */}
          {hasError && <div className="mt-4 bg-red-50 rounded-xl p-4">
              <div className="flex items-start">
                ...
                <div className="ml-3">
                  ...
                  <button onClick={retryAnalysis} className="mt-2 inline-flex items-center px-3 py-1.5 border border-transparent text-xs font-medium rounded-md text-white bg-red-600 hover:bg-red-700 focus:outline-none">
