...
export const HomeScreen: React.FC<HomeScreenProps> = ({
  ...
  const [meals, setMeals] = useState([{
    ...
    cookingTime: '10 min',
    time: '3:30 PM',
    image: 'https://images.unsplash.com/photo-1505252585461-04db1eb84625?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=2000&q=80',
    completed: false,
    progress: {
      hasIngredients: true,
      hasRecipe: true,
  ...
  return <div className="flex flex-col min-h-screen bg-[#F8F9FA] max-w-[430px] mx-auto">
      ...
      <main className="flex-1 overflow-hidden">
        {isFirstTimeUser ? <MealPrepSetup onComplete={() => setIsFirstTimeUser(false)} /> : <>
            ...
            <div className="relative" onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd}>
              ...
              <div className="flex justify-center mt-2 space-x-2">
                <button onClick={() => setActiveTab('nutrition')} className={`px-4 py-1 rounded-full text-sm font-medium transition-all duration-300 ${activeTab === 'nutrition' ? 'bg-[#4CAF50] text-white' : 'bg-gray-100 text-gray-500'}`}>
                  Nutrition
                </button>
                <button onClick={() => setActiveTab('health')} className={`px-4 py-1 rounded-full text-sm font-medium transition-all duration-300 ${activeTab === 'health' ? 'bg-[#4CAF50] text-white' : 'bg-gray-100 text-gray-500'}`}>
                  Health
                </button>
              </div>
              ...
              <div className="relative h-[320px] overflow-hidden">
                ...
                <div className={`absolute inset-0 transition-all duration-300 ease-in-out transform ${activeTab === 'nutrition' ? 'translate-x-0 opacity-100' : 'translate-x-[-100%] opacity-0'}`}>
                  <div className="mx-6 mt-3 p-5 bg-white rounded-2xl shadow-sm border border-gray-100">
                    <div className="flex items-center justify-between mb-6">
                      ...
                      <div className="relative w-20 h-20">
                        <svg className="w-full h-full -rotate-90" viewBox="0 0 100 100">
                          ...
                          <circle cx="50" cy="50" r="45" fill="none" stroke="#2196F3" strokeWidth="6" strokeDasharray="282.7" strokeDashoffset="84.81" strokeLinecap="round" className="transition-all duration-500" />
                        </svg>
                    ...
                    icon: '🥑',
                    progress: 60
                  }].map(stat => <div key={stat.label} className="bg-[#F8F9FA] rounded-xl p-4 text-center">
                          <div className="relative w-12 h-12 mx-auto mb-2">
                            ...
                            <svg className="w-full h-full -rotate-90 absolute inset-0" viewBox="0 0 48 48">
                              ...
                              <circle cx="24" cy="24" r="20" fill="none" stroke={stat.color} strokeWidth="4" strokeDasharray={`${stat.progress * 1.26} 126`} strokeLinecap="round" className="transition-all duration-700" />
                            </svg>
                          </div>
                ...
                {/* Health Dashboard */}
                <div className={`absolute inset-0 transition-all duration-300 ease-in-out transform ${activeTab === 'health' ? 'translate-x-0 opacity-100' : 'translate-x-[100%] opacity-0'}`}>
                  <div className="mx-6 mt-3">
                    <HealthDashboard onSwipe={() => setActiveTab('nutrition')} />
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto pb-24">
              ...
              const mealItems = meals.filter(meal => meal.type === mealType);
              return <div key={mealType}>
                      ...
                      <div className="space-y-4">
                        {mealItems.map((meal, index) => <MealRow key={index} meal={meal} onChecklistClick={() => {
                    const newMeals = [...meals];
                    const mealIndex = meals.indexOf(meal);
                    newMeals[mealIndex].completed = !meal.completed;
                    setMeals(newMeals);
                    showToast(!meal.completed ? 'Meal unmarked' : 'Meal logged successfully');
                  }} onCreateRecipe={() => {
      ...
      <nav className="fixed bottom-0 left-0 right-0 max-w-[430px] mx-auto bg-white border-t border-gray-100 z-20">
        <div className="flex justify-around items-center py-4 px-6 relative">
          <button className="flex flex-col items-center p-2 text-[#4CAF50]">
            <HomeIcon size={24} className="transition-transform hover:scale-110 duration-200" />
            <span className="text-xs mt-1 font-medium">Home</span>
          </button>
          <button className="flex flex-col items-center p-2 text-[#757575] hover:text-[#1A1A1A] transition-colors duration-200" onClick={() => navigateTo('meal-prep')}>
            <CalendarIcon size={24} className="transition-transform hover:scale-110 duration-200" />
            <span className="text-xs mt-1 font-medium">Meal Prep</span>
          </button>
          {/* Plus Button with Menu - Improved positioning and options */}
          <div className="relative">
            <button onClick={() => setShowPlusOptions(!showPlusOptions)} className="w-14 h-14 bg-[#1A1A1A] rounded-full flex items-center justify-center text-white shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-105 -mt-5">
              {showPlusOptions ? <XIcon size={24} className="animate-spin-once" /> : <PlusIcon size={24} className="transition-transform hover:rotate-90 duration-300" />}
            </button>
            {showPlusOptions && <div className="fixed left-1/2 transform -translate-x-1/2 bg-white rounded-xl shadow-xl border border-gray-100 w-64 overflow-hidden animate-scale-up z-30" style={{
            bottom: '80px'
          }}>
                <button className="w-full px-4 py-4 flex items-center text-left hover:bg-gray-50 border-b border-gray-100" onClick={() => {
              ...
              setShowPlusOptions(false);
                ...
                <button className="w-full px-4 py-4 flex items-center text-left hover:bg-gray-50" onClick={() => {
          ...
          <button className="flex flex-col items-center p-2 text-[#757575] hover:text-[#1A1A1A] transition-colors duration-200" onClick={() => navigateTo('progress')}>
            <BarChartIcon size={24} className="transition-transform hover:scale-110 duration-200" />
            <span className="text-xs mt-1 font-medium">Progress</span>
          </button>
        </div>
      </nav>
      {/* Overlay when plus menu is open */}
      {showPlusOptions && <div className="fixed inset-0 bg-black bg-opacity-30 z-10 animate-fade-in" onClick={() => setShowPlusOptions(false)}></div>}
    </div>;
};