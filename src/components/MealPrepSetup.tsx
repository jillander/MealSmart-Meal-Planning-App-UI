...
export const MealPrepSetup: React.FC<MealPrepSetupProps> = ({
  onComplete
}) => {
    ...
    switch (step) {
      case 1:
        return <div className="space-y-4 animate-fade-in">
            ...
            {[2, 3, 4, 5].map(number => <button key={number} onClick={() => {
            setPreferences(prev => ({
              ...prev,
              mealsPerDay: number
            }));
      ...
      case 2:
        return <div className="space-y-4 animate-fade-in">
            ...
            {dietTypes.map(diet => <button key={diet.id} onClick={() => {
            setPreferences(prev => ({
              ...prev,
              dietType: diet.id
            }));
      ...
      case 3:
        return <div className="space-y-4 animate-fade-in">
            ...
            <div className="grid grid-cols-2 gap-3">
              {cuisineTypes.map(cuisine => <button key={cuisine.id} onClick={() => handleCuisineToggle(cuisine.id)} className={`p-4 rounded-xl border ${preferences.cuisine.includes(cuisine.id) ? 'border-[#4CAF50] bg-[#4CAF50]/10' : 'border-gray-100 bg-white'} transition-all duration-200`}>
                  <div className="flex flex-col items-center text-center">
                    <span className="text-2xl mb-2">{cuisine.emoji}</span>
                    <span className="text-sm font-medium">{cuisine.name}</span>
                    {preferences.cuisine.includes(cuisine.id) && <CheckIcon size={16} className="text-[#4CAF50] mt-2" />}
                  </div>
                </button>)}
            </div>
            <button onClick={() => setStep(4)} className="w-full mt-4 py-3 bg-[#4CAF50] text-white rounded-full font-medium flex items-center justify-center">
              Continue
      ...
      case 4:
        return <div className="space-y-4 animate-fade-in">
            ...
            {[3, 5, 7].map(dayCount => <button key={dayCount} onClick={() => {
            setPreferences(prev => ({
              ...prev,
              days: dayCount
            }));
      ...
      case 5:
        return <div className="space-y-4 animate-fade-in">
              ...
              <ul className="space-y-2 text-sm">
                ...
                <li className="flex justify-between">
                  ...
                  <span className="font-medium">
                    {dietTypes.find(d => d.id === preferences.dietType)?.name || 'Standard'}
                  </span>
                </li>
                <li className="flex justify-between">
                  <span className="text-[#757575]">Preferred cuisines:</span>
                  <span className="font-medium">
                    {preferences.cuisine.length > 0 ? preferences.cuisine.map(c => cuisineTypes.find(ct => ct.id === c)?.name).join(', ') : 'Any'}
                  </span>
                </li>
                <li className="flex justify-between">
            ...
            <button onClick={onComplete} className="w-full py-4 bg-[#4CAF50] text-white rounded-full font-medium text-lg shadow-sm hover:bg-opacity-90 transition-colors">
              Start My Meal Plan
            </button>
          </div>;
      default:
