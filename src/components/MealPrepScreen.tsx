...
const initialMonthData = Array.from({
  ...
  day: index + 1,
  label: new Date(Date.now() + index * 24 * 60 * 60 * 1000).toLocaleDateString(
    'en-US',
    {
      weekday: 'short'
    }
...
export const MealPrepScreen: React.FC<MealPrepScreenProps> = ({
  ...
  const { completed: completedDays, total: totalDays } = getProgress();
  ...
  const handleDragEnd = (result: any) => {
    ...
    const sourceMeals = newMealData[sourceDay].meals.filter(
      (m) => m.type.toLowerCase() === sourceType
    );
    const destMeals = newMealData[destDay].meals.filter(
      (m) => m.type.toLowerCase() === destType
    );
    ...
    newMealData[sourceDay].meals = [
    ...newMealData[sourceDay].meals.filter(
      (m) => m.type.toLowerCase() !== sourceType
    ),
    ...sourceMeals];

    newMealData[destDay].meals = [
    ...newMealData[destDay].meals.filter(
      (m) => m.type.toLowerCase() !== destType
    ),
    ...destMeals];
      ...
      <DragDropContext onDragEnd={handleDragEnd}>
        <div className="flex flex-1">
          ...
          <div className="w-20 bg-white border-r border-gray-100">
            {Array.from({
              length:
              viewType === 'monthly' ? 30 : viewType === 'weekly' ? 7 : 1
            }).map((_, index) => {
              const date = new Date();
          ...
          <div className="flex-1">
            ...
            <div className="p-6 border-b border-gray-100 bg-white">
              ...
              <div className="flex space-x-1.5">
                ...
                <div
                  key={index}
                  className={`h-1.5 flex-1 rounded-full transition-all duration-300 ${index < completedDays ? 'bg-[#4CAF50]' : 'bg-[#E2E8F0]'}`} />
