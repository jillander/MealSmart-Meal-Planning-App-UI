...
import { ArrowLeftIcon, FilterIcon, ClockIcon, ChevronRightIcon, XIcon, BookmarkIcon, PlusCircleIcon, StarIcon, HeartIcon, InfoIcon, ShoppingBagIcon } from 'lucide-react';
...
export const RecipeSuggestionScreen: React.FC<RecipeSuggestionScreenProps> = ({
  ...
  const [recipes, setRecipes] = useState<Recipe[]>([{
    ...
    image: 'https://images.unsplash.com/photo-1567337710282-00832b415979?ixlib=rb-1.2.1&auto=format&fit=crop&w=1000&q=80',
    matchPercentage: 85,
    prepTime: '20 min',
    calories: 380,
  ...
  const cuisineTypes = ['all', 'Mediterranean', 'Asian', 'Italian', 'Mexican', 'American'];
  ...
  const filteredRecipes = recipes.filter(recipe => {
    ...
    if (activeFilters.length > 0 && !recipe.dietaryTags.some(tag => activeFilters.includes(tag))) return false;
    return true;
  });
  const toggleLikeRecipe = (id: string) => {
    setRecipes(recipes.map(recipe => recipe.id === id ? {
      ...recipe,
      liked: !recipe.liked
    } : recipe));
  };
  return <div className="flex flex-col min-h-screen bg-[#F8F9FA]">
      ...
      <header className="bg-white px-6 py-4 border-b border-gray-100">
        <div className="flex items-center justify-between">
          <div className="flex items-center">
            <button onClick={() => navigateTo('ingredient-confirmation')} className="mr-3">
          ...
          <button onClick={() => setShowFilters(true)} className="w-10 h-10 flex items-center justify-center rounded-full bg-gray-100 hover:bg-gray-200 transition-colors">
            <FilterIcon size={20} />
          </button>
        </div>
      </header>
      ...
      <div className="bg-white border-b border-gray-100 sticky top-0 z-10">
        <div className="px-6 py-3 overflow-x-auto scrollbar-hide">
          <div className="flex space-x-2">
            {cuisineTypes.map(cuisine => <button key={cuisine} onClick={() => setSelectedCuisine(cuisine)} className={`px-4 py-2 rounded-full text-sm font-medium whitespace-nowrap transition-all duration-200 ${selectedCuisine === cuisine ? 'bg-[#4CAF50] text-white shadow-md' : 'bg-gray-100 text-[#757575] hover:bg-gray-200'}`}>
      ...
      <div className="flex-1 overflow-y-auto p-6">
        {filteredRecipes.length === 0 ? <div className="flex flex-col items-center justify-center h-64 text-center">
          ...
          setActiveFilters([]);
            ...
            {filteredRecipes.map(recipe => <div key={recipe.id} className="bg-white rounded-xl border border-gray-100 overflow-hidden shadow-sm hover:shadow-md transition-all duration-200">
                <div className="relative">
                  ...
                  <button onClick={() => toggleLikeRecipe(recipe.id)} className="absolute top-3 right-3 w-8 h-8 bg-white rounded-full flex items-center justify-center shadow-md">
                    <HeartIcon size={16} className={`${recipe.liked ? 'text-red-500 fill-red-500' : 'text-gray-400'} transition-colors`} />
                  </button>
                ...
                <div className="p-4">
                  ...
                  {recipe.missingIngredients.length > 0 && <div className="mb-4 p-2 bg-[#FFF3E0] rounded-lg">
                      <p className="text-sm text-[#FF9800] flex items-start">
                        <ShoppingBagIcon size={14} className="mr-1 mt-0.5 flex-shrink-0" />
                        <span>
                  ...
                  <div className="flex flex-wrap gap-2 mb-4">
                    {recipe.dietaryTags.map(tag => <span key={tag} className="px-2 py-1 bg-gray-100 rounded-full text-xs text-[#64748B]">
                        {tag}
                      </span>)}
                  </div>
                  <div className="flex space-x-2">
                    <button onClick={() => navigateTo('recipe-detail')} className="flex-1 py-2.5 bg-[#4CAF50] text-white rounded-lg text-sm font-medium flex items-center justify-center hover:bg-[#43A047] transition-colors">
                      View Recipe
                    </button>
      ...
      {/* Filter Modal */}
      {showFilters && <div className="fixed inset-0 bg-black bg-opacity-50 flex items-end z-50 animate-fade-in">
          <div className="bg-white w-full rounded-t-2xl p-6 animate-slide-up">
            ...
            <div className="mb-6">
              ...
              <div className="flex flex-wrap gap-2">
                {['Match %', 'Prep Time', 'Calories', 'Rating'].map(sortOption => <button key={sortOption} className={`px-4 py-2 rounded-full text-sm font-medium ${sortOption === 'Match %' ? 'bg-[#4CAF50] text-white' : 'bg-gray-100 text-[#757575] hover:bg-gray-200'} transition-all duration-200`}>
                      {sortOption}
                    </button>)}
              </div>
            </div>
